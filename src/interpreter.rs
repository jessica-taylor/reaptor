...
enum InterpreterAction<'a> {
    ExecStatement(&'a VMStatement<usize>),
    PushBlock(BlockFrame<'a>),
}
...
impl<'a> Interpreter<'a> {
    ...
    fn step(&mut self) -> Result<(), String> {
        ...
        let act: InterpreterAction<'a> = match block_frame {
            BlockFrame::Block(ix, stmts) => {
                ...
                let stmt = &stmts[*ix];
                *ix += 1;
                InterpreterAction::ExecStatement(stmt)
            }
            BlockFrame::WhileCond(ix, cond, body) => {
                if *ix >= cond.len() {
                    if self.word_stack.pop().ok_or("No value")?.0 != 0 {
                        InterpreterAction::PushBlock(BlockFrame::WhileBody(0, cond, body))
                    } else {
                        call_frame.block_frames.pop();
                        return Ok(());
                    }
        ...
        match act {
            InterpreterAction::PushBlock(b) => call_frame.block_frames.push(b),
            InterpreterAction::ExecStatement(stmt) => match stmt {
                VMStatement::DelWord => {
